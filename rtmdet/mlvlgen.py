...
class MlvlPointGenerator:
    ...
    def grid_priors(self, featmap_size, with_stride=False):
        assert self.num_levels == len(featmap_size)
        mlvl_priors = []
        for i in range(self.num_levels):
            priors = self.single_level_grid_priors(
                featmap_size[i], level_idx=i, with_stride=with_stride
            )
            mlvl_priors.append(priors)
        return mlvl_priors

    def single_level_grid_priors(
        self, featmap_size, level_idx, with_stride=False
    ):
        ...
        if not with_stride:
            ...
            stride_h = shift_yy.new_full((shift_yy.shape[0],), stride_h)
